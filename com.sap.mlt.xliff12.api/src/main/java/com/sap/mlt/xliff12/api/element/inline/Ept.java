 ...
 * sequence of native codes. Each Ept has a corresponding {@link Bpt} element
 * within the translation unit. These paired elements are related via their
 * {@link Rid} attributes. If the {@link Rid} attribute is not present (in a 1.0
 * document for example), the attribute {@link Id} is used to match both tags.
 * The required {@link Id} attribute is used to identify the Ept inline code.
 * The optional {@link com.sap.mlt.xliff12.api.attribute.Ts} attribute was
 * DEPRECATED in XLIFF 1.1. The optional {@link Crc} attribute allows a
 * verification of the data. The optional {@link Xid} attribute references a
 * {@link TransUnit} or {@link BinUnit}, through its {@link Id} attribute value,
 * which can contain any translatable text from the inline code. The optional
 * {@link EquivText} attribute specifies text to substitute in place of the
 * inline tag.
 * 
 * @author D049314
 */
public interface Ept extends InlineBase, TextFragment {

	/**
	 * The element's name.
	 */
	static final String NAME = "ept";
