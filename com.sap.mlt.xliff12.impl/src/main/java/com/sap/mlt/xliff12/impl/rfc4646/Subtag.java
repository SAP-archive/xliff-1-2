...
import com.sap.mlt.xliff12.impl.rfc4646.enums.Extlang;
 ...
 * A Lang tag subtag
 */
public final class Subtag implements Serializable, Cloneable,
		Comparable<Subtag> {
	...
	public Subtag(Language language) {
		this(Type.PRIMARY, language.name().toLowerCase(Locale.ENGLISH));
	}
	...
	private static String getRegionName(String name) {
		return name.startsWith("UN") && name.length() == 5 ? name.substring(2)
				: name;
	}
	...
	public Subtag(Variant variant) {
		this(Type.VARIANT,
				getVariantName(variant.name().toLowerCase(Locale.ENGLISH)));
	}
	...
	public Subtag(Extlang extlang) {
		this(Type.EXTLANG, extlang.name().toLowerCase(Locale.ENGLISH));
	}
	...
	public Subtag(Singleton singleton) {
		this(Type.SINGLETON, singleton.name().toLowerCase(Locale.ENGLISH));
	}
	...
	private static String toTitleCase(String string) {
		...
		if (string.length() == 0)
			return string;
		char[] chars = string.toLowerCase(Locale.ENGLISH).toCharArray();
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ ((name == null) ? 0 : name.toLowerCase(Locale.ENGLISH).hashCode());
