...
public class TransUnitImpl extends XliffElementImpl implements TransUnit {

	public TransUnitImpl(Id id, Source source) {
		super(NAME);
	...
	public TransUnitImpl(org.w3c.dom.Element element)
	...
	protected void assertAttributesValid(org.w3c.dom.Element element)
		...
		Assert.xliffAttrRestricted(element, true, true, false, Id.NAME,
				Approved.NAME, Translate.NAME, Reformat.NAME, DataType.NAME,
				...
				Style.NAME, ExStyle.NAME, ExType.NAME, MaxBytes.NAME,
				MinBytes.NAME, SizeUnit.NAME, MaxHeight.NAME, MinHeight.NAME,
				MaxWidth.NAME, MinWidth.NAME, CharClass.NAME);
	}

	@Override
	protected void cloneChildrenFrom(ElementImpl elem) {
		TransUnitImpl source = (TransUnitImpl) elem;
		...
		if (source.target != null) {
			target = (Target) source.target.clone();
			attach(target);
		}
	...
	public List<? extends Node> getChildren() {
		...
		children.add(source);
		if (segSource != null) {
			children.add(segSource);
		}
		if (target != null) {
			children.add(target);
		}
		children.addAll(context);
	...
	protected void processChildren(
		...
		while (iter.nextIsXliffElement()) {
			org.w3c.dom.Element element = iter.getXliffElement(Source.NAME,
					...
					CountGroup.NAME, PropGroup.NAME, Note.NAME, AltTrans.NAME);
			...
			if (Source.NAME.equals(name)) {
				...
				context.add(new AltTransImpl(element));
			}
		}
		while (iter.hasNext()) {
	...
	public void setContext(List<? extends Context> context) {
		...
		Assert.areInstances(context, "context", AltTransImpl.class,
	...
	public void setTarget(Target target) {
		Assert.isInstance(target, "target", TargetImpl.class);
		assertNotAttached(target);
		detach(this.target);
		this.target = target;
		attach(this.target);
	}
