...
public class Range extends SubtagSet {
	...
	public Range(String range, boolean extended) {
		super(parse(range, extended).getPrimary());
	...
	public Range toBasicRange() {
		if (getPrimary().getType() == Subtag.Type.WILDCARD) {
			return new Range("*");
		} else {
	...
	private boolean checkBasic() {
		Subtag current = getPrimary().getNext();
		while (current != null) {
	...
	public boolean matches(Lang lang, boolean extended) {
		...
		if (isBasic() && !extended) {
			...
			while (i.hasNext()) {
				icurrent = i.next();
				while (icurrent.getType() == Subtag.Type.WILDCARD
						&& i.hasNext())
					icurrent = i.next();
				// the range ends in a wildcard so it will match everything
				// beyond this point
				...
				while (e.hasNext()) {
					ecurrent = e.next();
					if (extended
							&& (ecurrent.getType().ordinal() < icurrent
									.getType().ordinal()))
						continue;
	...
	private static final Pattern p_range_component = Pattern
			.compile(range_component);
	...
	private static final Pattern p_privateuse = Pattern.compile("^"
			+ privateuse + "$");
	private static final Pattern p_grandfathered = Pattern
			.compile(grandfathered);
	...
	public static Range parse(String range, boolean extended) {
		if (!extended) {
			...
			if (m.find()) {
				...
				primary = new Subtag(first.equals("*") ? Subtag.Type.WILDCARD
						: Subtag.Type.SIMPLE, first.toLowerCase(Locale.ENGLISH));
				current = primary;
				Matcher n = p_range_component.matcher(therest);
				while (n.find()) {
					String name = n.group(1).toLowerCase(Locale.ENGLISH);
					current = new Subtag(
							name.equals("*") ? Subtag.Type.WILDCARD
									: Subtag.Type.SIMPLE, name, current);
				}
			}
			return new Range(primary);
		} else {
			...
			if (m.find()) {
				...
				for (String tag : tags) {
					if (current == null) {
						primary = new Subtag(tag.equals("*") ? Type.WILDCARD
								: Type.SINGLETON, tag, null);
						current = primary;
					} else {
						current = new Subtag(tag.equals("*") ? Type.WILDCARD
								: Type.PRIVATEUSE, tag, current);
					}
				}
				return new Range(primary);
			}
			...
			if (m.find()) {
				...
				for (String tag : tags) {
					if (current == null) {
						...
						current = new Subtag(tag.equals("*") ? Type.WILDCARD
								: Type.EXTLANG, tag, current);
					}
				}
				if (script != null && script.length() > 0)
					current = new Subtag(
							script.substring(1).equals("*") ? Type.WILDCARD
									: Type.SCRIPT, script.substring(1), current);
				if (region != null && region.length() > 0)
					current = new Subtag(
							region.substring(1).equals("*") ? Type.WILDCARD
									: Type.REGION, region.substring(1), current);
				if (variant != null && variant.length() > 0) {
					variant = variant.substring(1);
					tags = variant.split("-");
					for (String tag : tags)
						current = new Subtag(tag.equals("*") ? Type.WILDCARD
								: Type.VARIANT, tag, current);
				}
				if (extension != null && extension.length() > 0) {
					extension = extension.substring(1);
					tags = extension.split("-");
					current = new Subtag(tags[0].equals("*") ? Type.WILDCARD
							: Type.SINGLETON, tags[0], current);
					for (int i = 1; i < tags.length; i++) {
						String tag = tags[i];
						current = new Subtag(tag.equals("*") ? Type.WILDCARD
								: tag.length() == 1 ? Type.SINGLETON
				...
				if (privateuse != null && privateuse.length() > 0) {
					...
					for (int i = 1; i < tags.length; i++) {
						current = new Subtag(
								tags[i].equals("*") ? Type.WILDCARD
										: Type.PRIVATEUSE, tags[i], current);
					}
				}
				return new Range(primary);
			}
