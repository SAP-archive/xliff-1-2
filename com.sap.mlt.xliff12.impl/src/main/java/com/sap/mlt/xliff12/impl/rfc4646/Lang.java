...
public final class Lang extends SubtagSet {
	...
	private static Subtag init(Locale locale) {
		...
		if (!variant.isEmpty()) {
			Subtag variantSubTag = new Subtag(Type.VARIANT, variant,
					(c == null) ? primary : c);
			if (!variantSubTag.isValid()) {
				Subtag singlton = new Subtag(Type.SINGLETON, "x",
						(c == null) ? primary : c);
				new Subtag(Type.PRIVATEUSE, variant, singlton);
			}
		}
		return primary;
		// }
	...
	private Locale initLocale() {
		...
		if (variant != null && region != null)
			return new Locale(primary.toString(), region.toString(), variant
					.toString());
		else if (region != null)
	...
	/**
	 * Get the Extlang tag. If there are multiple extlang tags, this will return
	 * the first one. The rest can be retrieved by following Subtag.getNext()
	 */
	...
	/**
	 * Get the beginning of the extension section. This will return the first
	 * prefix subtag of the first set of extension subtags.
	 */
	...
	/**
	 * Get the beginning of the private-use section. This will return the x
	 * prefix subtag
	 */
	 ...
	 */
	public Lang clone() {
		return new Lang(getPrimary().clone());
	}
	...
	public Lang canonicalize() {
		...
		for (Subtag tag : ext) {
			...
			current = tag.getNext();
			while (current != null
					&& current.getType() == Subtag.Type.EXTENSION) {
		...
		if (m.find()) {
			...
			if (extension != null && extension.length() > 0) {
				...
				for (int i = 1; i < tags.length; i++) {
					String tag = tags[i];
					current = new Subtag(tag.length() == 1 ? Type.SINGLETON
